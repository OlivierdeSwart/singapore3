...
import { useToast } from "@/hooks/scaffold-eth/use-toast";
...
export default function ServiceMarketplace() {
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      ...
      toast({
        ...
        action: (
          <ToastAction altText="Copy ID" onClick={() => navigator.clipboard.writeText(data.requestId)}>
            Copy ID
          </ToastAction>
        ),
      });
  ...
  const handleSubmitContent = async () => {
    ...
    try {
      ...
      setJobs(prev => prev.map(job => (job.id === selectedJob ? { ...job, status: "in_progress" } : job)));
    ...
    try {
      ...
      toast({
        ...
        description: data.isValid
          ? "The content meets all requirements."
      ...
      if (data.isValid) {
        ...
        setJobs(prev => prev.map(job => (job.id === selectedJob ? { ...job, status: "completed" } : job)));
  ...
  const handleCreateAttestation = async () => {
    ...
    try {
        ...
        if (parsedBody.success && parsedBody.attestation) {
          toast({
            ...
            action: (
              <ToastAction
                altText="Copy ID"
                onClick={() => navigator.clipboard.writeText(parsedBody.attestation.attestationId)}
              >
                Copy ID
              </ToastAction>
            ),
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "Failed to create attestation. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(prev => ({ ...prev, createAttestation: false }));
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  Attestation ID: {attestationResult.attestation.attestationId}
                </p>
              </CardContent>
            </Card>
          )}
